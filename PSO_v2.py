...
class Graph:
	...
	def getRandomPaths(self, max_size):
		...
		initial_vertice= list_vertices[0]
	...
	def run(self):
		...
		for t in range(self.iterations):
			...
			if t % 20 == 0:
				...
				plt.draw()
				plt.pause(.001)
			self.gcost_iter.append(self.gbest.getCostPBest())
...
import folium
def plot_map(cities, route,name='map.html'):
	m = folium.Map(location=[-15.788497, -47.879873], zoom_start=4)
	points = []
	for i in range(len(route)):
		points.append((cities[route[i]][0], cities[route[i]][1]))
	points.append((cities[route[0]][0], cities[route[0]][1]))
	folium.PolyLine(points, color="red", weight=2.5, opacity=1).add_to(m)
	m.save(f'output_map/{name}')
	m
...
if __name__ == "__main__":
	...
	pso = PSO(graph, iterations=2000, size_population=2000, beta=0.45, alfa=0.5)
	pso.run() # runs the PSO algorithm
	pso.showsParticles() # shows the particles

	# shows the global best particle
	print('gbest: %s | cost: %d\n' % (pso.getGBest().getPBest(), pso.getGBest().getCostPBest()))
	plot_map(cities, pso.getGBest().getPBest(), 'map.html')